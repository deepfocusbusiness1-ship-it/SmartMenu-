...
// ─── Allowed owner emails (configure as needed) ──────────────────────────────
const ALLOWED_EMAILS: string[] = [
  // "dueno@mibar.com",
  // "admin@mibar.com",
];
// If ALLOWED_EMAILS is empty, any authenticated user can access.
// Populate this array with the owner's email(s) to restrict access.
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-600 focus:outline-none focus:border-amber-500 transition-colors"
              placeholder="dueno@mibar.com"
              required
            />
          </div>
          <div>
...
function StatsSection() {
  ...
  const cards = [
    {
      label: "Facturado hoy",
      value: stats ? formatARS(stats.totalFacturado) : "—",
      icon: TrendingUp,
    ...
    },
    {
      ...
      value: stats ? formatARS(stats.totalPropinas) : "—",
      icon: DollarSign,
    ...
    },
    {
      ...
      value: stats ? stats.cantidadPedidos.toString() : "—",
      icon: ShoppingBag,
      color: "text-sky-400",
      bg: "bg-sky-400/10",
...
function ProductRow({
  ...
  product: Producto;
  onUpdate: (id: string, changes: Partial<Producto>) => Promise<void>;
  ...
  const handleCancel = () => {
    setDraft({ nombre: product.nombre, descripcion: product.descripcion, precio: product.precio });
    setEditing(false);
  };
  ...
  return (
    <div className={`bg-slate-900 border rounded-xl overflow-hidden transition-all ${product.disponible ? "border-slate-700/60" : "border-slate-800 opacity-60"}`}>
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        {/* Disponible toggle */}
        <button
          onClick={() => onUpdate(product.id, { disponible: !product.disponible })}
          className="shrink-0"
        ...
        <div className="shrink-0 text-right">
          <p className="text-amber-400 font-bold text-sm">{formatARS(product.precio)}</p>
        </div>
          ...
          {editing ? (
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full mt-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-500"
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full mt-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-500 resize-none"
                />
              </div>
              <div>
                <label className="text-xs text-slate-500 uppercase tracking-wider">Precio (ARS)</label>
                <input
                  type="number"
                  value={draft.precio}
              ...
              <div className="flex gap-2">
                <button
                  onClick={handleSave}
                  disabled={saving}
                  className="flex-1 flex items-center justify-center gap-1.5 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-slate-950 font-bold text-sm rounded-lg py-2 transition-colors"
                >
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm rounded-lg py-2 transition-colors"
                >
                  <X size={14} /> Cancelar
                </button>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => onUpdate(product.id, { popular: !product.popular })}
                ...
                <button
                  onClick={() => onUpdate(product.id, { happy_hour: !product.happy_hour })}
                  className={`flex items-center gap-1 text-xs px-2.5 py-1 rounded-full border transition-colors ${
                    product.happy_hour
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 text-sm rounded-lg px-3 py-2 transition-colors"
                >
                  <X size={13} />
                </button>
              </div>
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          {[
            { key: "nombre", label: "Nombre *", placeholder: "Ej: Empanada de carne" },
            { key: "categoria", label: "Categoría *", placeholder: "Ej: Entradas" },
            { key: "precio", label: "Precio ARS *", placeholder: "1500", type: "number" },
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) => setForm((d) => ({ ...d, descripcion: e.target.value }))}
              rows={2}
              className="w-full mt-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-500 resize-none"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={saving}
            className="w-full flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-slate-950 font-bold rounded-xl py-3 transition-colors"
          >
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-sm rounded-xl px-4 py-2.5 transition-colors shrink-0"
        >
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-slate-900 border border-slate-700 rounded-xl pl-10 pr-4 py-2.5 text-white text-sm placeholder-slate-600 focus:outline-none focus:border-amber-500 transition-colors"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {filtered.length === 0 ? (
            <p className="text-center text-slate-600 py-12 text-sm">No se encontraron productos.</p>
          ) : (
            filtered.map((p) => (
              <ProductRow
                key={p.id}
      ...
      {showModal && (
        <NewProductModal onClose={() => setShowModal(false)} onCreated={fetchProductos} />
      )}
    </div>
  );
}
...
function ConfiguracionSection() {
  ...
  useEffect(() => {
    const fetchState = async () => {
      // Check if ANY product has happy_hour = true to infer global state
      const { data } = await supabase
        .from("productos")
        .select("happy_hour")
        .eq("happy_hour", true)
        .limit(1);
      setHappyHourActive((data ?? []).length > 0);
      setLoading(false);
    };
    fetchState();
  }, []);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-white">Configuración del local</h2>
        <p className="text-slate-500 text-sm mt-0.5">Ajustes globales del menú</p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-white font-semibold">Happy Hour global</p>
              <p className="text-slate-500 text-sm mt-0.5">
                Activa o desactiva el descuento 2×1 en <strong className="text-slate-400">todos</strong> los productos marcados con Happy Hour.
              </p>
            </div>
          </div>

          {loading ? (
            <Loader2 size={20} className="text-slate-500 animate-spin shrink-0 mt-1" />
          ) : (
            <button
              onClick={toggleHappyHour}
              disabled={saving}
              className="shrink-0 mt-1"
              title={happyHourActive ? "Desactivar Happy Hour" : "Activar Happy Hour"}
            >
              {happyHourActive ? (
        ...
        <div className={`mt-4 flex items-center gap-2 text-sm transition-all duration-300 ${saved ? "opacity-100" : "opacity-0"}`}>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
        <p className="text-xs text-slate-600 uppercase tracking-wider mb-3">Acceso y seguridad</p>
        <p className="text-slate-500 text-sm">
          Para restringir el acceso a este panel a emails específicos, editá el array{" "}
          <code className="text-amber-400 bg-slate-800 px-1.5 py-0.5 rounded text-xs">ALLOWED_EMAILS</code>{" "}
          al inicio de <code className="text-amber-400 bg-slate-800 px-1.5 py-0.5 rounded text-xs">OwnerDashboard.tsx</code>.
        </p>
      </div>
    </div>
  );
...
export default function OwnerDashboard() {
  const [session, setSession] = useState<ReturnType<typeof supabase.auth.getSession> extends Promise<infer T> ? T["data"]["session"] : never>(null);
